...
class TaskListViewController: UIViewController {
        ...
        if segue.identifier == "ComposeSegue" {
            if let composeNavController = segue.destination as? UINavigationController,
               let composeViewController = composeNavController.topViewController as? TaskComposeViewController {
                ...
                composeViewController.taskToEdit = sender as? Task
                composeViewController.onComposeTask = { [weak self] task in
                    self?.handleComposeTask(task)
                }
            }
        }
    }
    
    private func handleComposeTask(_ task: Task) {
    ...
    private func refreshTasks() {
        tasks = Task.getTasks().sorted {
            if $0.isComplete == $1.isComplete {
                return $0.isComplete ? ($0.completedDate! < $1.completedDate!) : ($0.createdDate < $1.createdDate)
            }
            return !$0.isComplete
        }
        emptyStateLabel.isHidden = !tasks.isEmpty
...
extension TaskListViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configure(with: task) { [weak self] updatedTask in
            self?.handleCompleteTask(updatedTask)
        }
        return cell
    }
    
    private func handleCompleteTask(_ updatedTask: Task) {
